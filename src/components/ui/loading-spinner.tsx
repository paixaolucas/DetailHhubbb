...
export function LoadingSpinner({ size = "md", text }: LoadingSpinnerProps) {
  ...
  return (
    <div className={`flex items-center justify-center ${size === "lg" ? "h-64" : "p-4"}`}>
      <div
        className={`${sizes[size]} border-[#009CD9] border-t-transparent rounded-full animate-spin`}
      />
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-center h-64 w-full">
      <div className="text-center space-y-3">
        <div className="w-10 h-10 border-[3px] border-[#009CD9] border-t-transparent rounded-full animate-spin mx-auto" />
