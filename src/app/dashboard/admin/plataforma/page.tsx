...
const DEFAULT_CONFIG: PlatformConfig = {
  ...
  emailSuporte: "suporte@detailerhub.com",
  comissao: "15",
};
  ...
  return (
    <div className="space-y-6">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all disabled:opacity-70 ${
            saved
              ...
              : "bg-[#006079] hover:bg-[#007A99] text-white hover:shadow-lg hover:shadow-[#007A99]/30"
          }`}
        >
      ...
      <div className="glass-card p-6">
        <h2 className="text-base font-semibold text-[#EEE6E4] mb-5">Feature Flags</h2>
            ...
            return (
              <div key={key} className="flex items-center justify-between py-3 border-b border-white/10 last:border-0">
                <div className="flex-1 pr-4">
                  <p className={`text-sm font-medium ${isDanger && isOn ? "text-red-400" : "text-[#EEE6E4]"}`}>
                    {label}
                    {isDanger && isOn && (
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-all duration-300 flex-shrink-0 ${
                    isOn
                      ? isDanger ? "bg-red-500" : "bg-[#006079]"
                      : "bg-white/5"
                  }`}
                >
                  <span
                    className={`absolute top-1 w-4 h-4 bg-white/5 rounded-full shadow-sm transition-all duration-300 ${
                      isOn ? "left-7" : "left-1"
                    }`}
                  />
                </button>
      ...
      {/* Platform Config */}
      <div className="glass-card p-6">
        <h2 className="text-base font-semibold text-[#EEE6E4] mb-5">Configurações Gerais</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-1.5">Nome da Plataforma</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-1.5">Email de Suporte</label>
            <input
              type="email"
      ...
      <div className="glass-card p-6 border border-red-500/20">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10">
            <div>
              <p className="text-sm font-medium text-[#EEE6E4]">Exportar dados</p>
              <p className="text-xs text-gray-500 mt-0.5">Exportar todos os dados da plataforma em CSV</p>
            </div>
            <button
              onClick={exportConfig}
              className="flex items-center gap-2 px-4 py-2 border border-white/10 hover:border-white/10 text-gray-400 hover:text-[#EEE6E4] rounded-xl text-sm transition-all hover:bg-[#006079]/10"
            >
          ...
          <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/10">
            <div>
              <p className="text-sm font-medium text-[#EEE6E4]">Limpar Cache</p>
              <p className="text-xs text-gray-500 mt-0.5">Reinicializa o cache da aplicação</p>
            </div>
            <button
              onClick={resetCache}
              disabled={resetting}
              className="flex items-center gap-2 px-4 py-2 border border-white/10 hover:border-white/10 text-gray-400 hover:text-[#EEE6E4] rounded-xl text-sm transition-all hover:bg-[#006079]/10 disabled:opacity-70"
            >
