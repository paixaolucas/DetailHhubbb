...
const TRIGGER_LABELS: Record<string, { label: string; color: string }> = {
  ...
  ON_SUBSCRIPTION:     { label: "Ao Assinar",       color: "text-violet-400 bg-violet-500/10 border-violet-500/20" },
...
function Skeleton() {
  return (
    <div className="space-y-4 animate-pulse">
      ...
      <div className="h-10 bg-gray-50 rounded-xl w-56" />
      {[...Array(3)].map((_, i) => (
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Sequências de Email</h1>
      ...
      {loading ? (
        <div className="space-y-3 animate-pulse">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="glass-card p-5 h-20 bg-white" />
          ))}
        </div>
      ) : sequences.length === 0 ? (
        <div className="glass-card p-16 text-center">
          <Mail className="w-10 h-10 text-gray-600 mx-auto mb-3" />
          <p className="text-gray-900 font-semibold mb-1">Nenhuma sequência ainda</p>
          <p className="text-gray-500 text-sm">Crie sua primeira sequência de email automática.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {sequences.map((seq) => {
            const trig = TRIGGER_LABELS[seq.trigger] ?? { label: seq.trigger, color: "text-gray-400 bg-white border-gray-200" };
            return (
              <div key={seq.id} className="glass-card p-5 hover:border-violet-200 transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {!seq.isActive && (
                        <span className="text-xs px-2 py-0.5 rounded-full border border-gray-200 text-gray-500 bg-white">
                          Inativo
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-gray-200 text-gray-600 hover:text-gray-900 hover:border-violet-200 hover:bg-violet-50 transition-all"
                      title="Broadcast"
                    >
                    ...
                    <button
                      ...
                      className="text-gray-500 hover:text-gray-900 transition-colors p-1.5 rounded-lg hover:bg-violet-50"
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">Nova Sequência</h2>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1.5">Gatilho</label>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setBroadcastSeq(null)} className="text-gray-500 hover:text-gray-900 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1.5">Assunto</label>
              <input
                type="text"
                value={bSubject}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1.5">Mensagem</label>
              <textarea
                value={bBody}
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-all"
              >
