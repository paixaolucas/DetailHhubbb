...
const TRIGGER_LABELS: Record<string, { label: string; color: string }> = {
  ...
  ON_SUBSCRIPTION:     { label: "Ao Assinar",       color: "text-[#009CD9] bg-[#007A99]/10 border-[#007A99]/20" },
  ON_LESSON_COMPLETE:  { label: "Aula Concluída",   color: "text-[#009CD9] bg-[#009CD9]/10 border-[#009CD9]/20" },
...
function Skeleton() {
  return (
    <div className="space-y-4 animate-pulse">
      ...
      <div className="h-10 bg-white/5 rounded-xl w-56" />
      {[...Array(3)].map((_, i) => (
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#EEE6E4]">Sequências de Email</h1>
      ...
      {communities.length > 1 && (
        <div className="relative w-56">
          ...
          >
            {communities.map((c) => (
              <option key={c.id} value={c.id} className="bg-white/5">
                {c.name}
              </option>
            ))}
          </select>
      ...
      {loading ? (
        ...
        <div className="glass-card p-16 text-center">
          ...
          <p className="text-[#EEE6E4] font-semibold mb-1">Nenhuma sequência ainda</p>
        ...
        <div className="space-y-3">
          {sequences.map((seq) => {
            ...
            return (
              <div key={seq.id} className="glass-card p-5 hover:border-[#009CD9]/30 transition-all">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <p className="text-sm font-semibold text-[#EEE6E4]">{seq.name}</p>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-white/10 text-gray-400 hover:text-[#EEE6E4] hover:border-[#009CD9]/30 hover:bg-[#006079]/10 transition-all"
                      title="Broadcast"
                    >
                    ...
                    <button
                      ...
                      className="text-gray-500 hover:text-[#EEE6E4] transition-colors p-1.5 rounded-lg hover:bg-[#006079]/10"
                      title={seq.isActive ? "Desativar" : "Ativar"}
                    >
                      {seq.isActive
                        ? <ToggleRight className="w-5 h-5 text-[#009CD9]" />
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-[#EEE6E4]">Nova Sequência</h2>
              <button onClick={() => setShowCreate(false)} className="text-gray-500 hover:text-[#EEE6E4] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1.5">Gatilho</label>
              <div className="relative">
                ...
                >
                  {Object.entries(TRIGGER_LABELS).map(([k, v]) => (
                    <option key={k} value={k} className="bg-white/5">{v.label}</option>
                  ))}
                </select>
                <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 pointer-events-none" />
              </div>
      ...
      {broadcastSeq && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-[#EEE6E4]">Broadcast</h2>
                <p className="text-xs text-gray-500 mt-0.5">Enviar email a todos os membros ativos</p>
              </div>
              <button onClick={() => setBroadcastSeq(null)} className="text-gray-500 hover:text-[#EEE6E4] transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-white/5 border border-white/10 hover:border-[#009CD9]/30 focus:border-[#009CD9] rounded-xl px-4 py-3 text-[#EEE6E4] placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-[#009CD9]/30 transition-all resize-none"
              />
            </div>
            <div className="flex gap-3 pt-1">
              <button
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-[#006079] hover:bg-[#007A99] disabled:opacity-50 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-all"
              >
