...
export default function BadgesPage() {
  ...
  const [newBadge, setNewBadge] = useState({ name: "", description: "", icon: "🏆", color: "#8B5CF6" });
    ...
    try {
      if (editingBadge) {
        ...
        if (d.success) {
          ...
          setNewBadge({ name: "", description: "", icon: "🏆", color: "#8B5CF6" });
    ...
    <div className="space-y-6">
      ...
      {communities.length > 1 && (
        <div className="relative w-56">
          <select
            ...
            className="w-full appearance-none bg-white border border-gray-200 rounded-xl px-4 py-2.5 pr-10 text-gray-900 text-sm focus:outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-400/30 transition-all"
          >
            {communities.map((c) => (
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 animate-pulse">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="glass-card p-5 h-32 bg-white" />
          ))}
        </div>
      ) : (
        <>
          ...
          {badges.length === 0 && (
            <div className="glass-card p-16 text-center">
              ...
              <p className="text-gray-900 font-semibold mb-1">Nenhum badge ainda</p>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">{editingBadge ? "Editar Badge" : "Criar Badge"}</h2>
              <button onClick={() => { setShowCreate(false); setEditingBadge(null); setNewBadge({ name: "", description: "", icon: "🏆", color: "#8B5CF6" }); }} className="text-gray-500 hover:text-gray-900 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-1.5">Ícone (emoji)</label>
                <input
                  type="text"
                  value={newBadge.icon}
                  ...
                  placeholder="🏆"
                  maxLength={4}
                  className="w-full bg-white border border-gray-200 hover:border-violet-200 focus:border-violet-400 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 text-2xl text-center focus:outline-none focus:ring-2 focus:ring-violet-400/30 transition-all"
                />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3 p-3 bg-white rounded-xl">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900">{newBadge.name || "Nome do Badge"}</p>
      ...
      {awardBadge && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card p-6 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">Conceder Badge</h2>
              <button onClick={() => setAwardBadge(null)} className="text-gray-500 hover:text-gray-900 transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex items-center gap-3 p-3 bg-white rounded-xl">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-900">{awardBadge.name}</p>
            ...
            <div>
              ...
              {members.length === 0 ? (
                ...
                <div className="relative">
                  ...
                  >
                    <option value="" className="bg-white">Escolha um membro...</option>
                    {members.map((m) => (
                      <option key={m.userId} value={m.userId} className="bg-white">
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-yellow-600 hover:bg-yellow-500 disabled:opacity-50 text-gray-900 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all"
              >
...
function BadgeCard({ badge, onAward, onEdit, canAward }: { badge: Badge; onAward: () => void; onEdit: () => void; canAward: boolean }) {
  return (
    <div className="glass-card p-5 hover:border-violet-200 transition-all group flex flex-col gap-3">
      <div className="flex items-start justify-between">
        ...
        {badge.communityId && (
          <button onClick={onEdit} className="p-1.5 text-gray-600 hover:text-violet-400 hover:bg-violet-500/10 rounded-lg transition-colors" title="Editar badge">
            <Pencil className="w-3.5 h-3.5" />
          </button>
        )}
      </div>
      <div className="flex-1">
        <p className="text-sm font-semibold text-gray-900 leading-snug">{badge.name}</p>
        <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{badge.description}</p>
      </div>
      <button
        onClick={onAward}
        disabled={!canAward}
        className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium border border-gray-200 hover:border-violet-200 text-gray-600 hover:text-gray-900 hover:bg-violet-50 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
      >
