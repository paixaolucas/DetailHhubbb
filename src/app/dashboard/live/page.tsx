...
const STATUS_COLORS: Record<string, string> = {
  ...
  ENDED: "bg-white/50/10 text-gray-400 border-gray-500/20",
...
function fieldClass() {
  return "w-full bg-white/5 border border-white/10 hover:border-[#009CD9]/30 focus:border-[#009CD9] rounded-xl px-4 py-3 text-[#EEE6E4] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#009CD9]/30 transition-all text-sm";
}
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[#EEE6E4]">Lives</h1>
        ...
        <button
          ...
          className="flex items-center gap-2 bg-gradient-to-r from-[#006079] to-[#007A99] hover:from-[#007A99] hover:to-[#009CD9] text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-all hover:shadow-lg hover:shadow-[#007A99]/30"
        >
      ...
      {showNewForm && (
        <form onSubmit={editingSession ? handleUpdate : handleCreate} className="glass-card p-6 space-y-4">
          <h2 className="text-base font-semibold text-[#EEE6E4]">{editingSession ? "Editar Live" : "Nova Sessão ao Vivo"}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {communities.length > 1 && (
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-1.5">Comunidade</label>
            ...
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-400 mb-1.5">Título da live *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1.5">Data e hora *</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-1.5">Limite de participantes</label>
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => { setShowNewForm(false); setEditingSession(null); }} className="px-5 py-2.5 border border-white/10 hover:border-white/10 rounded-xl text-sm text-gray-400 hover:text-[#EEE6E4] transition-all">
              Cancelar
            </button>
          </div>
        </form>
      ...
      {sessions.length === 0 && !showNewForm && (
        <div className="glass-card p-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-[#EEE6E4] mb-2">Nenhuma live agendada</h3>
...
function SessionCard({ session, onStatusChange, onDelete, onEdit, formatDate }: {
  ...
  const statusClass = STATUS_COLORS[session.status] ?? "bg-white/50/10 text-gray-400 border-gray-500/20";
  return (
    <div className={`glass-card p-5 hover:border-[#009CD9]/30 transition-all ${session.status === "LIVE" ? "border-red-500/30" : ""}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="font-semibold text-[#EEE6E4]">{session.title}</h3>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {session.status === "SCHEDULED" && (
            <>
              <button onClick={() => onEdit(session)} className="flex items-center gap-1.5 text-xs bg-white/5 text-gray-400 hover:bg-white/5 border border-white/10 px-2.5 py-1.5 rounded-xl font-medium transition-colors">
          ...
          {session.status === "LIVE" && (
            <button onClick={() => onStatusChange(session.id, "ENDED")} className="flex items-center gap-1.5 text-xs bg-white/50/10 text-gray-400 hover:bg-white/50/20 border border-gray-500/20 px-2.5 py-1.5 rounded-xl font-medium transition-colors">
