...
function Avatar({ user, size = "md" }: { user: Participant; size?: "sm" | "md" }) {
  ...
  return (
    <div
      className={`${sz} bg-gradient-to-br from-blue-600 to-cyan-500 rounded-xl flex items-center justify-center text-white font-bold flex-shrink-0`}
    >
      {getInitials(user)}
    </div>
  );
...
export default function MessagesPage() {
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex gap-0 rounded-2xl overflow-hidden border border-white/10 bg-chrome-950">
      {/* ── Conversation list ── */}
      <div
        className={`${
      ...
      >
        ...
        <div className="h-14 px-4 flex items-center gap-3 border-b border-white/10 flex-shrink-0">
          ...
          <h1 className="text-white font-semibold text-sm">Mensagens</h1>
        </div>
            ...
            <div className="p-3 border-t border-white/10 flex-shrink-0">
              <div className="flex items-end gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
                <textarea
                  ...
                  rows={1}
                  className="flex-1 bg-transparent text-sm text-white placeholder-gray-600 outline-none resize-none max-h-32 py-1"
                  style={{ fieldSizing: "content" } as React.CSSProperties}
                />
                <button
                  onClick={sendMessage}
                  disabled={!draft.trim() || sending}
                  className="w-8 h-8 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed rounded-xl flex items-center justify-center transition-colors flex-shrink-0"
                >
                  {sending ? (
