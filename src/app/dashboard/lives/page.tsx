...
const STATUS_CONFIG: Record<string, { label: string; color: string }> = {
  SCHEDULED: { label: "Agendada", color: "bg-[#007A99]/10 text-[#009CD9] border-[#007A99]/20" },
  LIVE: { label: "Ao Vivo", color: "bg-red-500/10 text-red-400 border-red-500/20" },
  ENDED: { label: "Encerrada", color: "bg-white/50/10 text-gray-400 border-gray-500/20" },
...
const RSVP_OPTIONS: { value: RsvpStatus; label: string; icon: React.ElementType; active: string; inactive: string }[] = [
  { value: "GOING",     label: "Vou",      icon: Check,       active: "bg-green-600 text-[#EEE6E4] border-green-500",       inactive: "border-white/10 text-gray-400 hover:border-green-500/50 hover:text-green-400" },
  { value: "MAYBE",     label: "Talvez",   icon: HelpCircle,  active: "bg-yellow-600 text-[#EEE6E4] border-yellow-500",     inactive: "border-white/10 text-gray-400 hover:border-yellow-500/50 hover:text-yellow-400" },
    ...
    <div className="space-y-6 max-w-4xl">
          ...
          <div className="space-y-3">
            {liveNow.map((session) => (
              <div key={session.id} className="flex items-center justify-between bg-white/5 rounded-xl px-4 py-3 border border-red-500/20">
                <div>
                  <p className="font-semibold text-[#EEE6E4] text-sm">{session.title}</p>
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-xl text-sm font-medium transition-all ${
              filter === value
                ...
                : "glass-card hover:border-[#009CD9]/30 text-gray-400 hover:text-[#EEE6E4]"
            }`}
          >
            {label}
          </button>
      ...
      {filtered.length === 0 ? (
        <div className="glass-card p-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-[#EEE6E4] mb-2">Nenhuma live encontrada</h3>
          <p className="text-gray-400 text-sm">Nenhuma live disponível no momento.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((session) => {
            const statusConf = STATUS_CONFIG[session.status] ?? { label: session.status, color: "bg-white/50/10 text-gray-400 border-gray-500/20" };
            return (
              <div key={session.id} className={`glass-card p-5 hover:border-[#009CD9]/30 transition-all ${session.status === "LIVE" ? "border-red-500/30" : ""}`}>
                  ...
                  <div className="flex-shrink-0">
                    ...
                    {session.status === "ENDED" && session.replayUrl && (
                      <a
                        ...
                        className="flex items-center gap-1.5 glass-card hover:border-[#009CD9]/30 text-gray-400 hover:text-[#EEE6E4] px-3 py-2 rounded-xl text-sm font-medium transition-all"
                      >
                        <PlayCircle className="w-4 h-4" />
                        Replay
                      </a>
