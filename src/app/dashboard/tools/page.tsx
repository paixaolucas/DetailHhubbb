  ...
  return (
    <div className="space-y-8">
      ...
      {Object.entries(byCategory).map(([category, catTools]) => {
        const catInfo = CATEGORY_LABELS[category] ?? { label: category, color: "text-gray-400 bg-gray-500/10 border-gray-500/20" };
        return (
          <div key={category}>
...
function ToolCard({ tool, featured = false }: { tool: any; featured?: boolean }) {
  const catInfo = CATEGORY_LABELS[tool.category] ?? { label: tool.category, color: "text-gray-400 bg-gray-500/10 border-gray-500/20" };
