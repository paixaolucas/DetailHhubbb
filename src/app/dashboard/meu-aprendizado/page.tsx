  ...
  return (
    <div
      ...
      className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${
        isCurrent ? "bg-blue-500/10 border border-blue-500/20" : "hover:bg-white/5"
      }`}
    >
      <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${
        lesson.completed ? "bg-green-500/20" : isCurrent ? "bg-blue-500/20" : "bg-white/5"
      }`}>
        {lesson.completed ? (
          <CheckCircle2 className="w-3.5 h-3.5 text-green-400" />
        ) : (
          <Icon className={`w-3.5 h-3.5 ${isCurrent ? "text-blue-400" : "text-gray-500"}`} />
        )}
      </div>

      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium truncate ${
          lesson.completed ? "text-gray-500 line-through" : isCurrent ? "text-white" : "text-gray-300"
        }`}>
          {lesson.title}
        </p>
      </div>
    ...
    <div className="border border-white/10 rounded-xl overflow-hidden">
      ...
      >
        <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${
          module.progress === 100 ? "bg-green-500/20" : "bg-white/5"
        }`}>
          {module.progress === 100 ? (
  ...
  if (error) {
    return (
      <div className="space-y-6 max-w-4xl">
        <h1 className="text-2xl font-bold text-white">Meu Aprendizado</h1>
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: "Horas estudadas", value: `${stats.hoursWatched}h`, icon: Clock, color: "text-blue-400 bg-blue-500/10" },
      ...
      {communities.length === 0 ? (
        <div className="glass-card p-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">Nenhuma comunidade ainda</h3>
          <p className="text-gray-400 text-sm mb-6">Você ainda não está inscrito em nenhuma comunidade.</p>
          <a href="/communities" className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all inline-block">
            ...
            <div className="p-5 border-b border-white/10">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-base font-semibold text-white">{community.communityName}</h2>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    <p className="text-lg font-bold text-white">{community.progress}%</p>
