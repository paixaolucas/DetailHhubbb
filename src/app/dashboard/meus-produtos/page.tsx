    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Total de Vendas", value: totalSales, icon: ShoppingBag, color: "text-blue-400 bg-blue-500/10" },
      ...
      {showNewForm && (
        <form onSubmit={handleCreate} className="glass-card p-6 space-y-4">
          <h2 className="text-base font-semibold text-white">Novo Produto</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value }))} className={`${fieldClass()} bg-chrome-800`}>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">O que está incluso (uma por linha)</label>
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => setShowNewForm(false)} className="px-5 py-2.5 border border-white/20 hover:border-white/40 rounded-xl text-sm text-gray-300 hover:text-white transition-all">
              Cancelar
            </button>
          </div>
        </form>
      ...
      {listings.length === 0 ? (
        <div className="glass-card p-16 text-center">
          ...
          <h3 className="text-xl font-semibold text-white mb-2">Nenhum produto ainda</h3>
        ...
        <div className="space-y-3">
          {listings.map((listing) => (
            <div key={listing.id} className="glass-card p-5 hover:border-white/20 transition-all group">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="font-semibold text-white group-hover:text-blue-300 transition-colors">{listing.title}</h3>
                  {listing.shortDesc && <p className="text-sm text-gray-400 mt-0.5">{listing.shortDesc}</p>}
                  <div className="flex items-center gap-4 mt-2">
                    <span className="text-base font-bold text-white">R$ {Number(listing.price).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</span>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  {listing.status !== "ARCHIVED" && (
                    <button onClick={() => toggleStatus(listing.id, listing.status)} className="p-1.5 text-gray-500 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition-colors" title={listing.status === "ACTIVE" ? "Pausar" : "Ativar"}>
