    ...
    <div className="animate-pulse space-y-4">
      <div className="bg-white/5 border border-white/10 rounded-xl p-6 flex items-start gap-5">
        ...
        <div className="flex-1 space-y-2.5">
          ...
          <div className="h-3 bg-white/10 rounded w-2/3 mt-3" />
...
export default function MemberProfilePage() {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <Link
          ...
          className="text-xs text-[#009CD9] hover:underline mt-2 inline-block"
        >
          &larr; Voltar ao feed
        </Link>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
      ...
      <Link
        href={`/community/${communitySlug}/feed`}
        className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-[#EEE6E4] transition-colors"
      >
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        <div className="flex items-start gap-5">
          ...
          {profile?.user.avatarUrl ? (
            ...
            <div className="w-20 h-20 rounded-2xl bg-[#006079]/80 flex items-center justify-center text-2xl font-bold text-white flex-shrink-0">
              {initials}
            </div>
          )}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-lg font-bold text-[#EEE6E4]">{fullName}</h1>
              <span className="text-xs bg-[#006079]/10 text-[#009CD9] px-2 py-0.5 rounded-full font-medium">
                {roleLabel}
              </span>
            </div>
        ...
        {profile?.interests && profile.interests.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-4">
            {profile.interests.map((interest) => (
              <span
                key={interest}
                className="text-xs bg-white/5 text-gray-400 px-2.5 py-1 rounded-full"
              >
                {interest}
              </span>
            ))}
          ...
          <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-white/10">
            {Object.entries(profile.socialLinks).map(([platform, url]) => (
              <a
                ...
                className="flex items-center gap-1 text-xs text-[#009CD9] hover:text-[#007A99] hover:underline"
              >
          ...
          posts.map((post) => {
            ...
            return (
              ...
              >
                {/* Space badge */}
                {post.space && (
                  <p className="text-xs text-[#009CD9] font-medium mb-1.5">
                    #{post.space.name}
                ...
                {post.title && (
                  <h3 className="text-sm font-semibold text-[#EEE6E4] mb-1 leading-snug">
                    {post.title}
                  </h3>
                )}
